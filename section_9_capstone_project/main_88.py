...
def calc_acum_avg(frame, accumulated_weight):
    global background
...
def segment(frame, threshold_min=25):
    ...
    if len(contours) == 0:
        ...
        # assuming the largest externel contour in ROI is the hand
...
def count_fingers(thresholded, hand_segment):
    ...
    distance = pairwise.euclidean_distances(
        [(cX, cY)], Y=[left, right, top, bottom])[0]
    max_distance = distance.max()
    radius = int(0.9*max_distance)
    circumfrence = (8*np.pi*radius)
...
while True:
    ...
    roi = frame[roi_top:roi_bottom, roi_bottom:roi_left]
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (7, 7), 0)

    if num_frames < 60:
        calc_acum_avg(gray, accumulated_weight)
        if num_frames <= 59:
            cv2.putText(frame_copy, 'Wait, getting background',
                        (200, 300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        if hand is not None:
            ...
            cv2.drawContours(
                frame_copy, [hand_segment+(roi_right, roi_top)], -1, (255, 0, 0), 5)
